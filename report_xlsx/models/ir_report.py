    ...
    * insert_rows(2, 10, above=True, copy_style=False)
    ...
    for c in self._cells.values():
        ...
        if c.row > row_idx:
            ...
            if old_coor in self.formula_attributes:
                ...
                fa = copy(self.formula_attributes[old_coor])
    ...
    if list(self.row_dimensions):
        for row in range(list(self.row_dimensions)[-1] + cnt, row_idx + cnt, -1):
            new_rd = copy(self.row_dimensions[row - cnt])
            new_rd.index = row
    ...
    for row in range(new_row_idx, new_row_idx + cnt):
        ...
        new_rd = copy(self.row_dimensions[row-1])
        new_rd.index = row
        self.row_dimensions[row] = new_rd
        ...
        for col in range(self.max_column):
            ...
            if fill_formulae and source.data_type == TYPE_FORMULA:
                ...
                if s_coor in self.formula_attributes and 'ref' not in self.formula_attributes[s_coor]:
                    fa = copy(self.formula_attributes[s_coor])
            ...
            source.font = copy(source_copy.font)
            source.alignment = copy(source_copy.alignment)
            source.border = copy(source_copy.border)
            source.fill = copy(source_copy.fill)
        ...
        for cr in self.merged_cells.ranges:
            ...
            if max_row == min_row == row_idx:
                for row in range(new_row_idx, new_row_idx + cnt):
                    ...
                    to_merge_cells.append(newCellRange)
        for mgc in to_merge_cells:
            self.merge_cells(mgc)
...
MultiCellRange.__contains__ = __contains__
